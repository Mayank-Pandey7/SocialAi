...
import {
  ...
  faCalendarDays, faBars, faXmark, faRightFromBracket,
...
const NAV_ITEMS = [
  ...
  { to: '/generator', icon: faWandMagicSparkles,   label: 'Generator' },
  { to: '/trending',  icon: faFire,                label: 'Trending'  },
  { to: '/analyzer',  icon: faChartBar,            label: 'Analyzer'  },
  ...
  return (
    <div style={styles.wrapper}>
      ...
      <aside
        ...
        style={{ ...styles.sidebar, width: collapsed ? 72 : 220 }}
      >
        {/* Logo */}
        ...
        <nav style={{ flex: 1, padding: '0.5rem 0' }}>
          {NAV_ITEMS.map(item => (
            <NavLink key={item.to} to={item.to}
              ...
              <FontAwesomeIcon icon={item.icon} style={{ fontSize: '1rem', flexShrink: 0 }} />
        ...
        <div style={styles.userFooter}>
          ...
          <button onClick={handleLogout} style={styles.iconBtn} title="Logout">
            <FontAwesomeIcon icon={faRightFromBracket} style={{ color: 'var(--text-muted)' }} />
          </button>
        </div>
      </aside>
...
const styles = {
  ...
  collapseBtn:  { marginLeft: 'auto', background: 'transparent', border: 'none', cursor: 'pointer', color: 'var(--text-muted)', fontSize: '0.9rem', padding: '0.25rem', flexShrink: 0 },
