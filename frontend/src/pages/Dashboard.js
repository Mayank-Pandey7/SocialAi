  ...
  <div className="card fade-in">
    <div className="stat-card">
      ...
      <div>
        ...
        {change && <div className={`stat-change ${change.startsWith('+') ? 'up' : 'down'}`}>{change} this week</div>}
      </div>
    </div>
  </div>
);
...
const QuickAction = ({ to, icon, title, desc, color }) => (
  <Link to={to} style={{ textDecoration:'none' }}>
    <div className="card" style={{ cursor:'pointer', display:'flex', gap:'1rem', alignItems:'flex-start' }}>
      <div style={{ width:44, height:44, borderRadius:10, background:color+'20', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'1.3rem', flexShrink:0 }}>{icon}</div>
      <div>
  ...
  return (
    <div>
      ...
      <div className="grid-2" style={{ marginBottom:'2rem' }}>
        <div>
          <h2 style={{ fontSize:'1rem', fontWeight:600, marginBottom:'1rem', color:'var(--text-secondary)', textTransform:'uppercase', letterSpacing:'0.05em' }}>Quick Actions</h2>
        ...
        {/* Recent Posts */}
        <div>
          <h2 style={{ fontSize:'1rem', fontWeight:600, marginBottom:'1rem', color:'var(--text-secondary)', textTransform:'uppercase', letterSpacing:'0.05em' }}>Recent Posts</h2>
          {loading ? (
            ...
            <div className="card" style={{ textAlign:'center', color:'var(--text-muted)', padding:'2rem' }}>
              ...
              <Link to="/generator" className="btn btn-primary" style={{ marginTop:'1rem', display:'inline-flex' }}>Generate Now</Link>
            </div>
          ) : (
            <div style={{ display:'flex', flexDirection:'column', gap:'0.75rem' }}>
              {posts.map(post => (
                <div key={post._id} className="card" style={{ padding:'1rem' }}>
                  ...
                  <div style={{ display:'flex', gap:'0.5rem', alignItems:'center' }}>
                    <span className={`badge ${post.status === 'scheduled' ? 'badge-yellow' : post.status === 'posted' ? 'badge-green' : 'badge-purple'}`}>{post.status}</span>
